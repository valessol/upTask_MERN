...
import { newTaskForm } from "../constants/forms";
import Alert from "./Alert";
import { useParams } from "react-router-dom";

const Modal = () => {
  const { modal, setModal, alert, setAlert, handleSubmitModal } = useProjects();
  ...
  useEffect(() => {
    if (modal.data?._id) {
      setValues({
        ...modal.data,
        finishDate: modal.data.finishDate.split("T")[0],
      });
    } else
      setValues({
        name: "",
  ...
  const handleSubmit = async (e) => {
    ...
    await handleSubmitModal({ ...values, project: params.id });
    setValues({
      name: "",
      description: "",
      priority: "",
